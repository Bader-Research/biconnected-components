...
int prefix_sum_max(int *buff, int n, THREADED)
{
  int * result,i,start,t=0;
  ...
  pardo(i,0,n,1){
	if(i!=start) buff[i]=max(buff[i],buff[i-1]);
	t=buff[i];
  }
  node_Barrier();
  result[MYTHREAD]=t;
  ...
  on_one_thread{
    for(i=1;i<THREADS;i++) result[i]=max(result[i],result[i-1]);
    for(i=THREADS-1;i>0;i--) result[i]=result[i-1];
    result[0]=-MAXINT;
  } 
  node_Barrier();
  ...
  t = result[MYTHREAD];
  pardo(i,0,n,1)
	if(MYTHREAD!=0) buff[i]=max(t,buff[i]);	
  node_Barrier();
  node_free(result,TH);
}
