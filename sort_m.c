...
{
	int a_min,a_max,b_min,b_max,t;
	
	a_min = a.v1; a_max = a.v2;
	b_min = b.v1; b_max = b.v2;
	if(a_min>a_max) {
		t=a_min;
		a_min=a_max;
		a_max=t;
	...
	if(a_min<b_min || (a_min==b_min && a_max<b_max))
		return 1;
	return 0;
...
#define put_int(a,i) (a).v1=(i)
#define get_int(a) (int)((a).v1)
...
if ((reps) && (rems <= (col_size/2)))
  {rems += M;
   reps--;
   Aux = (TYPE *) malloc(max(rems,M)*sizeof(TYPE));
   assert_malloc(Aux);
  }
else
  {Aux = (TYPE *) malloc(max(rems,M)*sizeof(TYPE));
  ...
  {if (rems > 2)
	...
	{tot <<= 1; 
	 ...
	 buffer2_ptr = temp_ptr;
      ...
      finish_ptr = dest_ptr;
      put_int(*(finish_ptr++),rems);  /* put_int*/
      done = 0;
      while (! done) 
	{if (myle((*s1_ptr), (*s2_ptr)))
	   {*(finish_ptr++) = *(s1_ptr++);
...
#define RIGHT(i)        (2*(i+1))
	...
	while(j<len_L[MYTHREAD])
	{
		...
		while(t < 2*THREADS-1){
			change_P[n++]=t;
			if(LEFT(t)<2*THREADS-1 && myeq(T[t],T[LEFT(t)])) t = LEFT(t);
			else t = RIGHT(t);
		}
		i = change_P[n-1];
		id = i-(THREADS-1);
...
/*
	...
	*/
	...
	node_free(TempT,TH);
	node_free(Source,TH);
	node_free(Sample1,TH);
	node_free(Sample2,TH);
	node_free(Splitters,TH);
	node_free(Counts,TH);
	node_free(Number,TH);
	free(Aux);
	node_Barrier();
